...
STATEMENTS = [
    # baseline
    ("SELECT name FROM duckdb.planets;", 9),
    # push limit
    ("SELECT name FROM duckdb.planets LIMIT 1;", 1),
    # test with filter
    ("SELECT name FROM duckdb.planets WHERE gravity > 1;", 8),
    # pushable filter and limit should push the limit
    ("SELECT name FROM duckdb.planets WHERE gravity > 1 LIMIT 1;", 1),
    # if we can't push the filter, we can't push the limit
    ("SELECT name FROM duckdb.planets WHERE SIGNUM(gravity) > 1 LIMIT 1;", 9),
    # we don't push past ORDER BY
    ("SELECT * FROM duckdb.planets ORDER BY name LIMIT 3", 9),
    # push past subqueries
    ("SELECT name FROM (SELECT * FROM duckdb.planets) AS S LIMIT 3", 3),
]

@pytest.mark.parametrize("query, expected_rows", STATEMENTS)
def test_duckdb_limit_pushdown(query, expected_rows):
...
if __name__ == "__main__":  # pragma: no cover
    ...
    for index, (statement, read_columns) in enumerate(STATEMENTS):
        ...
        try:
            ...
            test_duckdb_limit_pushdown(statement, read_columns)
            print(
