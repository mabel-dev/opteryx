...
def test_predicate_pushdowns_blobs_csv():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT user_name FROM testdata.flat.formats.csv WITH(NO_PARTITION) WHERE user_verified = TRUE;"
    )
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT user_name FROM testdata.flat.formats.csv WITH(NO_PARTITION) WHERE user_verified = TRUE and following < 1000;"
    )
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT user_name FROM testdata.flat.formats.csv WITH(NO_PARTITION) WHERE user_verified = TRUE and username LIKE '%b%';"
    )
...
if __name__ == "__main__":  # pragma: no cover
    ...
    test_predicate_pushdowns_blobs_csv()
    run_tests()
