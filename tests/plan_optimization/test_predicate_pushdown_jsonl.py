...
def test_predicate_pushdowns_blobs_jsonl():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT user_name FROM testdata.flat.formats.jsonl WITH(NO_PARTITION) WHERE user_verified = TRUE;"
    )
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT user_name FROM testdata.flat.formats.jsonl WITH(NO_PARTITION) WHERE user_verified = TRUE and following < 1000;"
    )
