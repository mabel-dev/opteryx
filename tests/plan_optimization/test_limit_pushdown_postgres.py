...
STATEMENTS = [
    # baseline
    ("SELECT name FROM pg.planets;", 9),
    # push limit
    ("SELECT name FROM pg.planets LIMIT 1;", 1),
    # test with filter
    ("SELECT name FROM pg.planets WHERE gravity > 1;", 8),
    # pushable filter and limit should push the limit
    ("SELECT name FROM pg.planets WHERE gravity > 1 LIMIT 1;", 1),
    # if we can't push the filter, we can't push the limit
    ("SELECT name FROM pg.planets WHERE SIGNUM(gravity) > 1 LIMIT 1;", 9),
    # we don't push past ORDER BY
    ("SELECT * FROM pg.planets ORDER BY name LIMIT 3", 9),
...
if __name__ == "__main__":  # pragma: no cover
    ...
    for index, (statement, read_columns) in enumerate(STATEMENTS):
        ...
        try:
            ...
            test_postgres_limit_pushdown(statement, read_columns)
            print(
