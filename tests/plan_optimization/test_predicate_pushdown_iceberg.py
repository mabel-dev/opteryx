...
test_cases = [
    (
        "SELECT user_name FROM iceberg.tweets WHERE user_verified = TRUE;",
        711,
        711
    ),
    (
        "SELECT user_name FROM iceberg.tweets WHERE user_verified = TRUE and following < 1000;",
        266,
        266
    ),
    (
        "SELECT user_name FROM iceberg.tweets WHERE user_verified = FALSE and followers > 1000;",
        28522,
        28522
    ),
    (
        "SELECT user_name FROM iceberg.tweets WHERE followers >= 5000;",
        7123,
        7123
    ),
    (
        "SELECT user_name FROM iceberg.tweets WHERE following <= 1;",
        831,
        831
    ),
    (
        "SELECT user_name FROM iceberg.tweets WHERE user_verified = TRUE and user_name = 'Dakota News Now';",
        1,
        1
    ),
    (
        "SELECT user_name FROM iceberg.tweets WHERE user_name = 'Dakota News Now';",
        1,
        1
    ),
    (
        "SELECT user_name FROM iceberg.tweets WITH(NO_PARTITION) WHERE user_verified = TRUE and user_name LIKE '%b%';",
        86,
        711
    ),
    (
        "SELECT * FROM iceberg.missions WHERE Lauched_at < '2000-01-01';",
        3014,
        3014
    ),
    (
        "SELECT * FROM iceberg.planets WHERE rotationPeriod = lengthOfDay;",
        3,
        9
    ),
    (
        "SELECT * FROM iceberg.planets WHERE gravity > 10;",
        2,
        9
    ),
]
