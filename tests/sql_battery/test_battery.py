...
STATEMENTS = [
        ...
        ("SELECT BOOLEAN(planetId - 3) FROM $satellites GROUP BY BOOLEAN(planetId - 3)", 2, 1),
        ("SELECT VARCHAR(planetId) FROM $satellites GROUP BY VARCHAR(planetId)", 7, 1),
        ("SELECT STR(planetId) FROM $satellites GROUP BY STR(planetId)", 7, 1),
        ("SELECT COUNT(*) FROM $satellites GROUP BY TIMESTAMP('2022-01-0' || VARCHAR(planetId))", 7, 1),
        ("SELECT NUMERIC(planetId) FROM $satellites GROUP BY NUMERIC(planetId)", 7, 1),
        ("SELECT INT(planetId) FROM $satellites GROUP BY INT(planetId)", 7, 1),
        ("SELECT INTEGER(planetId) FROM $satellites GROUP BY INTEGER(planetId)", 7, 1),
        ("SELECT FLOAT(planetId) FROM $satellites GROUP BY FLOAT(planetId)", 7, 1),
        ("SELECT CAST(planetId AS BOOLEAN) FROM $satellites", 177, 1),
        ("SELECT CAST(planetId AS VARCHAR) FROM $satellites", 177, 1),
        ("SELECT CAST('2022-01-0' || VARCHAR(planetId) AS TIMESTAMP) FROM $satellites", 177, 1),
...
#        ("SELECT * FROM testdata.schema WITH(NO_PARTITION, NO_PUSH_PROJECTION) ORDER BY 1", 2, 4),
        ...
        ("SELECT * FROM $satellites WHERE NULLIF(planetId, 5) IS NULL", 67, 8),
        ("SELECT * FROM $satellites WHERE NULLIF(planetId, 5) IS NOT NULL", 110, 8),
