...
def test_predicate_pushdowns_blobs_zstd():
    ...
    cur = conn.cursor()
    cur.execute(
        "SET enable_optimizer = false; SELECT user_name FROM testdata.flat.formats.zstd WITH(NO_PARTITION) WHERE user_verified = TRUE;"
    )
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT user_name FROM testdata.flat.formats.zstd WITH(NO_PARTITION, NO_PUSH_SELECTION) WHERE user_verified = TRUE;"
    )
    # if we disable pushdown, we read all the rows from the source and we do the filter
    assert cur.rowcount == 711, cur.rowcount
    assert cur.stats["rows_read"] == 100000, cur.stats
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT user_name FROM testdata.flat.formats.zstd WITH(NO_PARTITION) WHERE user_verified = TRUE;"
    )
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT user_name FROM testdata.flat.formats.zstd WITH(NO_PARTITION) WHERE user_verified = TRUE and following < 1000;"
    )
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT user_name FROM testdata.flat.formats.zstd WITH(NO_PARTITION) WHERE user_verified = TRUE and user_name LIKE '%b%';"
    )
