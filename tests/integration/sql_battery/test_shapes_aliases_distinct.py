...
STATEMENTS = [
        ...
        ("SELECT P_1.* FROM $planets AS P_1 INNER JOIN $satellites AS P_2 ON P_1.id = P_2.planet_id AND P_2.id != 5", 176, 20, None),
        ...
        ("SELECT * FROM $planets INNER JOIN $satellites ON $planets.name <> $satellites.name", 1593, 28, None),
        ("SELECT * FROM $planets CROSS JOIN $satellites WHERE $planets.name != $satellites.name", 1593, 28, None),
        ("SELECT * FROM $planets INNER JOIN $satellites WHERE $planets.name != $satellites.name", 1593, 28, SqlError),
        ("SELECT * FROM $planets INNER JOIN $satellites ON $planets.name != $satellites.name", 1593, 28, None),
