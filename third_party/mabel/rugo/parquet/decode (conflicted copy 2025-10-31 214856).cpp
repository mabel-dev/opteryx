...
static DecodedColumn DecodeColumnFromChunk(const uint8_t* file_data, size_t file_size,
  ...
  try {
    ...
    // Adjust chunk-relative offsets when writers store relative positions
    int64_t chunk_start = target_col->column_chunk_file_offset;
    int64_t dictionary_offset = target_col->dictionary_page_offset;
    int64_t data_offset = target_col->data_page_offset;

    if (chunk_start >= 0) {
      if (dictionary_offset >= 0 && dictionary_offset < chunk_start) {
        dictionary_offset += chunk_start;
      }
      if (data_offset >= 0 && data_offset < chunk_start) {
        data_offset += chunk_start;
      }
    }

    // Attempt to load dictionary page if offset is provided
    std::cerr << "DEBUG: Checking for dictionary page, raw_offset=" << target_col->dictionary_page_offset
              << " adjusted_offset=" << dictionary_offset
              << " chunk_start=" << chunk_start << std::endl;

    if (dictionary_offset >= 0 && (uint64_t)dictionary_offset < file_size) {
      const uint8_t* dict_ptr = file_data + dictionary_offset;
    ...
    if (data_offset < 0 || (uint64_t)data_offset >= file_size) {
      return result;
    }

    const uint8_t* current_ptr = file_data + data_offset;
    size_t remaining_size = 0;
    if (target_col->total_compressed_size > 0) {
      uint64_t max_size = file_size - data_offset;
      remaining_size = std::min<uint64_t>(target_col->total_compressed_size, max_size);
    } else {
      remaining_size = file_size - data_offset;
    }
