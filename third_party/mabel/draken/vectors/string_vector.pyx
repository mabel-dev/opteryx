...
from libc.stdint cimport int32_t, intptr_t, int8_t, uint8_t, uint64_t
...
cdef class StringVectorBuilder:
    ...
    @classmethod
    def from_source(cls, StringVector source, double growth_factor=1.25, Py_ssize_t extra_bytes=0):
        """Create a resizable builder sized relative to an existing ``StringVector``.

        Parameters
        ----------
        source:
            Input vector used to infer row count and baseline byte usage.
        growth_factor:
            Multiplicative slack applied to the source byte usage. Must be >= 1.0.
        extra_bytes:
            Absolute slack added after multiplication to accommodate expansions.
        """
        ...
        cdef Py_ssize_t length = source.ptr.length
        cdef Py_ssize_t base_bytes = 0
        if length > 0 and source.ptr.offsets != NULL:
            base_bytes = source.ptr.offsets[length]

        initial_val = base_bytes
        if base_bytes > 0:
            scaled_val = int(base_bytes * growth_factor)
            if scaled_val > initial_val:
                initial_val = scaled_val

        if extra_bytes > 0:
            initial_val += extra_bytes

        if initial_val <= 0:
            initial_val = max(length, 64)
        elif initial_val > sys.maxsize:
            initial_val = sys.maxsize

        if initial_val < length:
            initial_val = length

        cdef Py_ssize_t initial = <Py_ssize_t>initial_val
