...
pushes LIMIT operations before projections that contain expensive calculations,
reducing the number of rows that need to be processed by the projection.
"""
...
class LimitPushdownStrategy(OptimizationStrategy):
        ...
        if node.node_type == LogicalPlanStepType.Project:
            ...
            for limit_node in context.collected_limits:
                if self._can_push_before_projection(limit_node, node):
                    # This limit can be pushed before the projection
    ...
    def _can_push_before_projection(
        ...
        """
        Determine if a LIMIT node can be pushed before a projection.

        LIMIT nodes can always be pushed before projections since they have no
        column dependencies - they simply reduce the number of rows.

        Args:
            limit_node: The LIMIT node to potentially push
            project_node: The PROJECT node to potentially push past

        Returns:
            True if the limit can be pushed before the projection
        """
        # LIMIT nodes can always be pushed before projections
        return limit_node.node_type == LogicalPlanStepType.Limit
