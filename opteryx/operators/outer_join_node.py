...
- Cython buffers: Uses IntBuffer for efficient index collection
"""
...
class OuterJoinNode(JoinNode):
    def __init__(self, properties: QueryProperties, **parameters):
        ...
        self.right_buffer = []
        self.right_schema = None  # Store right relation schema
        self.left_relation = None
        self.empty_right_relation = None
        self.left_hash = None
        self.left_seen_flags = None  # numpy array for tracking matched rows
        self.matched_count = 0  # Track how many left rows have been matched
        ...
        if join_leg == "left":
            if morsel == EOS:
                ...
                if self.join_type == "left outer":
                    ...
                    # Initialize seen_flags array for tracking matched rows
    ...
    def _process_left_outer_join_morsel(self, morsel: pyarrow.Table):
        ...
        for h, right_rows in right_hash.hash_table.items():
            ...
            # Early termination: if all left rows are matched, we can stop tracking
            # (but still need to process remaining right data for correctness)
