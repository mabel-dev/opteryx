...
class TestHLL(unittest.TestCase):
    ...
    def test_id(self):
        for sketch in self.sketches:
            self.assertEqual(hll.jaccard_index(sketch, sketch), 1.0)
            self.assertTrue(abs(sketch.report() - 3.0) < 0.1)
...
class TestHMH(unittest.TestCase):
    ...
    def test_id(self):
        for sketch in self.sketches:
            self.assertEqual(hmh.jaccard_index(sketch, sketch), 1.0)
            self.assertTrue(abs(sketch.getcard() - 50.0) < 6.0)
