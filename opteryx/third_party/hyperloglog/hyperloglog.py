...
class HyperLogLog(object):
    ...
    def __init__(self, p=8, reg=None, hashfunc=CityHash64):
        if reg is None:
            self.p = p
            self.m = 1 << p
            self.reg = numpy.zeros((self.m,), dtype=numpy.int8)
        else:
            # Check if the register has the correct type
            if not isinstance(reg, numpy.ndarray):
    ...
    def count(self):
        ...
        e = self.alpha * float(self.m ** 2) / numpy.sum(2.0 ** (-self.reg))
        ...
        if e <= small_range_threshold:
            num_zero = self.m - numpy.count_nonzero(self.reg)
    ...
    def merge(self, other):
        ...
        self.reg = numpy.maximum(self.reg, other.reg)
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.p == other.p
            and self.m == other.m
            and numpy.array_equal(self.reg, other.reg)
        )
    ...
    def _linearcounting(self, num_zero):
        return self.m * numpy.log(self.m / float(num_zero))

    def _largerange_correction(self, e):
        return -(1 << 32) * numpy.log(1.0 - e / (1 << 32))
    ...
    def union(cls, *hyperloglogs):
        ...
        reg = numpy.maximum.reduce([h.reg for h in hyperloglogs])
        h = cls(reg=reg)
        return h
    ...
    def deserialize(cls, buf):
        ...
        try:
            h.reg = numpy.array(
                struct.unpack_from("%dB" % h.m, buf, offset), dtype=numpy.int8
            )
        except TypeError:
            h.reg = numpy.array(
    ...
    def __setstate__(self, buf):
        ...
        try:
            self.reg = numpy.array(
                struct.unpack_from("%dB" % self.m, buf, offset), dtype=numpy.int8
            )
        except TypeError:
            self.reg = numpy.array(
...
class HyperLogLogPlusPlus(HyperLogLog):
    ...
    def _estimate_bias(self, e, p):
        ...
        nearest_neighbors = numpy.argsort((e - estimate_vector) ** 2)[:6]
        return numpy.mean(bias_vector[nearest_neighbors])
