...
from opteryx.engine.planner.expression import ExpressionTreeNode, NodeType
...
class QueryPlanner(ExecutionTree):
    ...
    def _build_filters(self, filters):
        ...
        if "Between" in filters:
            ...
            if inverted:
                ...
                left_node = ExpressionTreeNode(
                    NodeType.OPERATOR, value="<", left_node=expr, right_node=low
                )
                right_node = ExpressionTreeNode(
                    NodeType.OPERATOR, value=">", left_node=expr, right_node=high
                )
            else:
        ...
        if "InList" in filters:
            left = self._build_filters(filters["InList"]["expr"])
            right = (
                {self._build_filters(v)[0] for v in filters["InList"]["list"]},
                TOKEN_TYPES.LIST,
            )
        ...
        if "Nested" in filters:
            return (self._build_filters(filters["Nested"]),)
