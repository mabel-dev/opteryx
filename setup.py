...
if SHOULD_BUILD_EXTENSIONS:
    ...
    def make_draken_extension(module_path, source_file, depends=None, language=None, **kwargs):
        ...
        ext_kwargs = {
            ...
            "include_dirs": include_dirs + ["third_party/mabel/draken", "third_party/cyan4973"],
            "depends": depends,
        }
    ...
    extensions = [
        ...
        Extension(
            name="opteryx.compiled.simd_probe",
            sources=[
...
""")
    ...
    for ext in extensions:
        if ext.name.startswith("opteryx.draken.vectors."):
            ...
            # Ensure the cpu features probe is compiled into draken vector
            # extensions that may use SIMD helpers.
            if "src/cpp/cpu_features.cpp" not in ext.sources:
                ext.sources.append("src/cpp/cpu_features.cpp")
            ext.language = "c++"
